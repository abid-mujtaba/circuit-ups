   ...
   \begin{center}
      ...
      \begin{figure}[h!]
         ...
         \begin{circuitikz}
            ...
            \coordinate (dc_gnd) at (1, 0);            % We define a coordinate named 'dc_gnd' at the position (1, 0) so now (dc_gnd) is equivalent to (1, 0)
            ...
            \draw (dc_gnd) to[short, o-] (3, 0);          % A similar line is drawn below it from the dc_gnd node to (3, 0)


            \draw (0.25, 3) node[] {$+\SI{12}{\volt}$ DC};      % We place free floating text as nodes at specified location
            \draw (0.25, 2.75) node[] {(Input)};

            \draw (dc_gnd) node[left=1ex] {DC GND};             % We place the "DC GND" label as a node to the left of the dc_gnd = (1, 0) position and specify that an additional 1ex space be inserted between the node and the specified position
