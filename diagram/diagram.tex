   ...
   \begin{center}
      ...
      \begin{figure}[h!]
         ...
         \begin{circuitikz}
            ...
            \draw (1, 0) to[short, o-] (3, 0);          % A similar line is drawn below it from the (1, 0) node to (3, 0)


            \draw (1, 3) node[left=1ex, name=dc_in] {$+\SI{12}{\volt}$ DC};         % We want to place the text "12V DC" to the left of (1, 3) so we specify 'left=1ex' which places the node to the left of (1, 3) and adds an additional 1ex of space. We also specify a name for the node 'dc_in' which we use below to refer to it
            \draw (dc_in) node[below] {(Input)};            % We want the text '(Input)' to appear just below the '12V DC' text so we use the name of the node defined above, 'dc_in', as the position for this node but specify that this node should be drawn below the specified position. This allows us to place this text relative to existing nodes
